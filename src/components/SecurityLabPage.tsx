  ...
  return (
    <div className="p-6 max-w-7xl mx-auto space-y-6 font-sans">
      ...
      <div className="bg-gradient-to-r from-red-950/40 via-[#0A0F1D] to-[#0A0F1D] border border-red-500/30 rounded-xl p-6 shadow-xl relative overflow-hidden">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 relative z-10">
          <div>
            ...
            <h1 className="text-2xl sm:text-3xl font-black text-white tracking-tight flex items-center gap-3">
              ...
              <span className="text-xs font-mono font-normal px-2 py-1 bg-slate-900 border border-slate-700 text-slate-300 rounded">
                Pitch Centerpiece
              </span>
            </h1>
        ...
        {/* Quick KPI Bar */}
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mt-6 pt-4 border-t border-slate-800/80 text-xs font-mono">
          <div className="bg-slate-950/60 p-2.5 rounded border border-slate-800">
            <span className="text-slate-400 text-[10px] uppercase">Attack Vectors</span>
            <p className="text-sm font-bold text-white mt-0.5">4 Scenarios (Sec 7)</p>
          </div>
          <div className="bg-slate-950/60 p-2.5 rounded border border-slate-800">
            <span className="text-slate-400 text-[10px] uppercase">Defense Rate</span>
            <p className="text-sm font-bold text-emerald-400 mt-0.5">
              {totalExecuted > 0 ? `${totalBlocked} / ${totalExecuted} Blocked (100%)` : 'Ready to Test'}
            </p>
          </div>
          <div className="bg-slate-950/60 p-2.5 rounded border border-slate-800">
            <span className="text-slate-400 text-[10px] uppercase">Price Invariant</span>
            <p className="text-sm font-bold text-red-400 mt-0.5">Schema Rejected (Inv 2)</p>
          </div>
          <div className="bg-slate-950/60 p-2.5 rounded border border-slate-800">
            <span className="text-slate-400 text-[10px] uppercase">Audit Logging</span>
            <p className="text-sm font-bold text-[#3395FF] mt-0.5">Zero Silent Paths (Inv 4)</p>
          </div>
        </div>
      </div>
              ...
              <button
                ...
                className={`p-4 rounded-xl text-left border transition-all relative overflow-hidden flex flex-col justify-between ${
                  isSelected
                    ...
                    : 'bg-[#0A0F1D] border-slate-800 hover:border-slate-700 hover:bg-[#0D1426]'
                }`}
              >
                <div>
                ...
                <div className="pt-3 border-t border-slate-800/80 flex items-center justify-between text-[11px] font-mono">
                  <span className="text-slate-500">Target: {spec.target_sku}</span>
      ...
      <div className="grid grid-cols-12 gap-6">
        ...
        <div className="col-span-12 lg:col-span-5 space-y-4">
          <div className="bg-[#0A0F1D] border border-slate-800 rounded-xl p-5 space-y-4">
            ...
            <div>
              ...
              <textarea
                ...
                rows={6}
                className="w-full bg-[#05070A] border border-slate-800 rounded-lg p-3 text-xs font-mono text-red-300 focus:outline-none focus:border-red-500/50 leading-relaxed"
              />
        ...
        <div className="col-span-12 lg:col-span-7 space-y-4">
          {activeResult ? (
            <div className="space-y-4">
              ...
              <div className="bg-[#0A0F1D] border border-slate-800 rounded-xl p-5 space-y-3">
                <div className="flex items-center justify-between pb-3 border-b border-slate-800">
                  <div className="flex items-center gap-2">
                  ...
                  {activeResult.audit_log && (
                    <button
                      ...
                      className="px-2 py-1 bg-slate-900 hover:bg-slate-800 text-slate-300 rounded text-[10px] font-mono flex items-center gap-1 border border-slate-800 transition-colors"
                    >
                ...
                <p className="text-xs text-slate-400 leading-relaxed">
                  Every attack block writes exactly one immutable row to the <code className="text-slate-200 bg-slate-900 px-1 py-0.5 rounded font-mono">audit_log</code> table in SQLite. No silent paths, ever.
                </p>

                {activeResult.audit_log ? (
                  <div className="bg-[#05070A] border border-slate-800/80 rounded-lg p-3 font-mono text-xs space-y-2">
                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 text-[11px] pb-2 border-b border-slate-800">
                      <div>
                        <span className="text-slate-500 block text-[9px] uppercase">Row ID</span>
                        <span className="text-[#3395FF] font-bold">#{activeResult.audit_log.id}</span>
                      </div>
                      <div>
                      ...
                      <div>
                        ...
                        <span className="text-slate-300 text-[10px]">{activeResult.audit_log.timestamp}</span>
                      </div>
                    </div>

                    <div>
                      <span className="text-slate-500 block text-[9px] uppercase mb-0.5">Audit Reason</span>
                      <p className="text-slate-200 text-[11px] bg-slate-900/50 p-2 rounded border border-slate-800 leading-relaxed">
                        {activeResult.audit_log.reason}
                      </p>
                    </div>

                    <div>
                      <span className="text-slate-500 block text-[9px] uppercase mb-0.5">Payload Snapshot (Stored in DB)</span>
                  ...
                  <div className="p-4 bg-slate-900/30 rounded border border-slate-800 text-xs text-slate-500 font-mono">
            ...
            <div className="bg-[#0A0F1D] border border-dashed border-slate-800 rounded-xl p-12 text-center flex flex-col items-center justify-center space-y-3 min-h-[380px]">
