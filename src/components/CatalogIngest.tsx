...
export const CatalogIngest: React.FC<CatalogIngestProps> = ({
  ...
  return (
    <div className="flex flex-col space-y-5">
      ...
      <div className="bg-[#0A0F1D] border border-slate-800 rounded-lg p-4">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            <div className="flex items-center gap-2">
              ...
              <span className="text-[10px] font-mono px-2 py-0.5 rounded bg-blue-500/10 text-[#3395FF] border border-blue-500/20">
                POST /catalog/upload
              </span>
            </div>
          ...
          <div className="flex items-center gap-3">
            <div className="bg-[#05070A] border border-slate-800 px-3 py-1.5 rounded text-right">
              <div className="text-[10px] uppercase font-mono text-slate-500">Locked Items</div>
              <div className="text-sm font-bold font-mono text-emerald-400">{lockedCount}</div>
            </div>
            <div className="bg-[#05070A] border border-slate-800 px-3 py-1.5 rounded text-right">
              <div className="text-[10px] uppercase font-mono text-slate-500">Held-Out (Rejects)</div>
              <div className="text-sm font-bold font-mono text-rose-400">{heldOutCount}</div>
            </div>
            <div className="bg-[#05070A] border border-blue-500/30 px-3.5 py-1.5 rounded text-right">
              <div className="text-[10px] uppercase font-mono text-[#3395FF]">Coverage %</div>
        ...
        <div className="mt-3">
          <div className="flex justify-between text-[11px] font-mono text-slate-400 mb-1">
            ...
            <span className="text-slate-300 font-bold">{coveragePercent}%</span>
          </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-12 gap-4">
        ...
        <div
          ...
          className={`md:col-span-7 border-2 border-dashed rounded-lg p-6 flex flex-col items-center justify-center cursor-pointer transition-all ${
            isDragOver
              ...
              : 'border-slate-800 hover:border-slate-700 bg-[#0A0F1D]/50'
          }`}
        >
          <input
            type="file"
          ...
          />
          <Upload className="w-8 h-8 text-[#3395FF] mb-2" />
        ...
        <div className="md:col-span-5 bg-[#0A0F1D] border border-slate-800 rounded-lg p-4 flex flex-col justify-between">
          <div>
            <div className="flex items-center gap-2 text-xs font-bold font-mono text-slate-300 uppercase tracking-wider mb-2">
      ...
      <div className="bg-[#05070A] border border-blue-500/20 rounded-lg p-3 text-xs font-mono">
        <div className="flex items-center gap-2 text-[#3395FF] font-bold mb-1">
          <ShieldCheck className="w-4 h-4" />
          <span>INVARIANT 1 CODE PATH SEPARATION PROOF</span>
        </div>
        <p className="text-slate-400 text-[11px] leading-relaxed">
          <strong className="text-slate-200">Zero LLM Price Visibility:</strong> Groq is restricted strictly to mapping messy column headers (passed as string list <code className="text-amber-400">headers: List[str]</code>) and cleaning title text.
      ...
      <div className="grid grid-cols-1 gap-5">
        ...
        <div className="border border-slate-800 rounded-lg bg-[#0A0F1D] overflow-hidden">
          ...
          <div className="overflow-x-auto max-h-[300px]">
            <table className="w-full text-left border-collapse text-xs font-mono">
              <thead className="sticky top-0 bg-[#0A0F1D] text-[10px] text-slate-400 uppercase tracking-wider border-b border-slate-800">
                <tr>
              ...
              <tbody className="divide-y divide-slate-800/60">
                {displayLockedItems.length === 0 ? (
                  <tr>
                    <td colSpan={8} className="px-4 py-6 text-center text-slate-500 font-mono">
                  ...
                  displayLockedItems.map((item, idx) => (
                    <tr key={`${item.sku}-${idx}`} className="hover:bg-slate-800/40 transition-colors">
                      ...
                      <td className="px-4 py-2 text-slate-200 max-w-xs truncate">{item.title || item.name}</td>
                      ...
                      <td className="px-4 py-2 text-center text-slate-300">{item.stock}</td>
                      ...
                      <td className="px-4 py-2 text-right">
                        {onQuickCheckout && (
                          <button
                            ...
                            className="px-2 py-1 rounded bg-[#3395FF]/20 hover:bg-[#3395FF]/30 text-[#3395FF] border border-[#3395FF]/40 text-[10px] font-bold uppercase transition-colors"
                          >
                            Pay 1x
                          </button>
                        )}
        ...
        <div className="border border-rose-900/40 rounded-lg bg-[#0A0F1D] overflow-hidden">
                ...
                {displayRejectedItems.length === 0 ? (
                  ...
                  displayRejectedItems.map((item, idx) => (
                    <tr key={`${item.source_row_ref}-${idx}`} className="hover:bg-rose-950/20 transition-colors">
                      ...
                      <td className="px-4 py-2 text-slate-400">{item.raw_sku || 'N/A'}</td>
                      <td className="px-4 py-2 text-slate-200 max-w-xs truncate">{item.raw_title}</td>
